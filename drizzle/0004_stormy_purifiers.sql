...
CREATE TABLE `__new_attempts` (
	...
	CONSTRAINT "attempts_outcome_check" CHECK("__new_attempts"."outcome" IS NULL OR "__new_attempts"."outcome" IN ('succeeded', 'failed', 'cancelled', 'escalated')),
	CONSTRAINT "attempts_completed_outcome_check" CHECK(("__new_attempts"."state" = 'completed') = ("__new_attempts"."outcome" IS NOT NULL)),
	CONSTRAINT "attempts_session_id_check" CHECK("__new_attempts"."state" NOT IN ('session_created', 'running') OR "__new_attempts"."devin_session_id" IS NOT NULL),
