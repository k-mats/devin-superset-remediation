CREATE TABLE `verifications` (
	...
	CONSTRAINT "verifications_kind_check" CHECK("verifications"."kind" IN ('command', 'github_checks')),
	CONSTRAINT "verifications_status_check" CHECK("verifications"."status" IN ('passed', 'failed', 'unverified', 'error'))
);
--> statement-breakpoint
CREATE INDEX `verifications_attempt_head_kind_index` ON `verifications` (`attempt_id`,`head_sha`,`kind`);--> statement-breakpoint
