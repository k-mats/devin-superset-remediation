...
import { getAttempt, setVerificationCandidate } from '../db/task-state.js';
...
function main(): number {
  ...
  if (!values.attempt || commands.length === 0) {
    console.error(
      'Usage: verification-propose --attempt <id> --command "<cmd>" [--command "<cmd2>"] [--shell bash|sh]'
    );
    return 1;
  }
  ...
  try {
    ...
    console.log(script);
    console.log(
      `Approve with: verification-approve --attempt ${values.attempt} --spec-hash ${spec.sha256}`
    );
    return 0;
  } finally {
    closeDb();
